...
class PPNet(nn.Module):
    ...
    def forward(self, x):
        ...
        distances = torch.reshape(
            distances, (distances.size()[0], distances.size()[1], -1)
        )
        k = int(top_k_percentage * distances.size()[2] / 100)
        top_k_min_distances, _ = torch.topk(-distances, k=k, dim=2)
        top_k_min_distances = -top_k_min_distances
